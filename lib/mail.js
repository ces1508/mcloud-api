...
export default {
 ...
 async sendMultiple (campaingId) {
  ...
  if (priceCampaign <= user.balanceEmail ) {
    ...
    let descontar = await r.db('mepscloud').table('users').get(user.id).update({balanceEmail: discount})
      ...
      if (contacts.length > 0) {
        ...
        for (let j = 0; j  < contacts.length; j++) {
          ...
          if (contacts[j].email) {
            ...
            await r.db('mepscloud').table('users').get(user.id).upadte({balanceEmail: balance}).run(conn)
          }
        }
 ...
 async makeRequest (from, sender, subject, template, personalizations, userId, price) {
   let request = sg.emptyRequest({
    ...
    request.body = {
     ...
     .catch(function (err) {
       console.error(err)
        r.connect({host: '104.131.43.30',port: 28015}, function (conn) {
          r.db('mepscloud').table('users').get(userId).run(conn, function (user) {
            let  balance = ((personalizations.length * price) + user.balanceEmail)
            r.db('mepscloud').table('users').get(userId).upadte({balanceEmail: balance}).run(conn, function (data, err) {
              if (err) throw err
              console.log('incrementado salgo')
            })
          })
